...
class AdvancedUI(UI):
    ...
    def list_all_notes_ui(self):
        st.header("List All Notes")
...
def main():
    note_manager = NoteManager()  # Initialisering af NoteManager
    ui = AdvancedUI(note_manager)  # Brug af AdvancedUI med NoteManager

    st.title("Notes App")
    menu_choice = st.sidebar.radio("Menu", ["Create To-Do Note", "Create Meeting Note", "List All Notes", "Create Note"])

    if menu_choice == "Create Note":
        ui.create_note_ui()
    elif menu_choice == "Create To-Do Note":
        ui.create_todo_ui()  # Kald af create_todo_ui metoden
    elif menu_choice == "Create Meeting Note":
        ui.create_meeting_ui()  # Kald af create_meeting_ui metoden
    elif menu_choice == "List All Notes":
        AdvancedUI.list_all_notes_ui()
